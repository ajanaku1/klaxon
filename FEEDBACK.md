...
Integration feedback for two sponsor stacks Klaxon depends on. The
KeeperHub section (Part 1) is the formal submission for the
[KeeperHub Builder Feedback Bounty](https://ethglobal.com/events/openagents/prizes/keeperhub)
($500, up to 2 teams at $250 each). The 0G Compute section (Part 2) is
here because we hit enough friction that writing it down would have
saved us a day, and the issues are independent of KeeperHub.

Built during the ETHGlobal Open Agents hackathon (Apr 25 to May 3, 2026).
Each item is written so a new user can grasp the issue from the first
paragraph and someone familiar with the stack can act on the technical
detail in the same item.
...
Klaxon is a decentralized exploit-protection swarm. Three independent AI agents watch a protected protocol, each one running a Python process on its own AXL (Gensyn) node. When a suspicious oracle move shows up, each agent signs a Finding object with its own secp256k1 key, gossips it across the AXL mesh, and a per-node aggregator counts signatures. Once any agent sees three distinct authorized signatures over the same finding hash, that agent races the others to submit `Guardian.pause(sigs[], findingHash, teeAttestationHash)` on chain. Whichever agent's tx lands first wins; the others revert harmlessly with `AlreadyProcessed`.

KeeperHub is what gets the pause tx onto chain reliably (private routing, retries, gas handling). After a successful pause, an x402 V2 session settles bounty payouts to the three contributing agents.
...
**Plain English.** Klaxon was originally targeting 0G Chain, one of the sponsor chains for this same hackathon. KeeperHub does not list 0G as a supported network and there is no public way to add a custom chain through the API or UI. We had to redeploy our Solidity stack to Base Sepolia to get a KeeperHub workflow running, then design a cross-chain story so the 0G iNFT track requirements were still met. That redesign cost about half a day.
...
- 0G Galileo is a plain EVM chain. Adding it should be a config-only change (RPC, chainId, explorer URL, a flag for whether your private mempool is supported there). Not being able to self-add it forced an architectural compromise on us.

**What would unblock other builders at this same event.**

1. Add 0G Galileo (chainId 16602, RPC `https://evmrpc-testnet.0g.ai`, explorer `https://chainscan-galileo.0g.ai`) to the catalog. It is one of three sponsor chains for ETHGlobal Open Agents.
2. Document an "add custom chain" path for users with a plain RPC URL. Even without private routing on that chain, builders can still use KeeperHub for orchestration and scheduling.
3. In the chain-picker UI, show a short note next to each chain: "private mempool yes/no" so builders know up front whether the private-routing feature applies.

---

## Issue 2: 404 page-not-found is returned with HTTP 200 status code from API endpoints

**Plain English.** When you hit an API path that does not exist, KeeperHub returns the website's 404 HTML page instead of a JSON error. From a script's perspective the request looks fine (200 OK, body is HTML), so debugging tools default to thinking the call worked. We spent five extra minutes trying to figure out whether `/api/networks` was just empty before realizing the response was the 404 page.
...
The fix is in your Next.js API router. A catch-all 404 handler at `/api/*` that returns a JSON error (not HTML) with the right status code would surface this immediately. Same fix probably applies to `/api/node-types`, `/api/integrations` (when no integrations exist), and any other unmounted route.

---

## Issue 3: Workflow node JSON shape is not in any public docs

**Plain English.** To create a workflow programmatically you need to know the exact JSON shape for a "Write Contract" action node, including the field names KeeperHub uses for chainId, function selector, ABI, args, signer, and gas overrides. None of this is in the public docs we found. We reverse-engineered the shape by reading workflows we created in the visual builder.

**Technical detail.** The example payloads on the GitHub README ([KeeperHub/keeperhub](https://github.com/KeeperHub/keeperhub), [KeeperHub/mcp](https://github.com/KeeperHub/mcp)) show the API endpoints and tool names but never the schema for a node's `data.config`. After listing existing workflows via `GET /api/workflows` we got a sample for the `trigger` type:
...
1. Publish a JSON Schema for each node type at `/api/node-types/<typeId>/schema` (or similar). A TypeScript `.d.ts` file in the public repo would also work.
...
3. The visual builder already serializes to this format internally. A "Show JSON" button on each node in the builder would let users copy-paste.
...
**Plain English.** KeeperHub lets you build a workflow where a webhook receives JSON from outside and a contract action uses fields from that JSON as its arguments. The intended way to wire a webhook field into a contract argument is a Mustache-style template like `{{@webhook-trigger.data.sigs}}`. In our testing, those templates are not expanded before the workflow tries to JSON-parse the `functionArgs` string. The literal characters `{{` reach the parser, JSON parsing fails on the second `{`, and the action errors out with `Invalid function arguments JSON: Expected property name or '}' in JSON at position 2`. We could not get any template variant to work, so we fell back to a workaround: every time the agent wants to fire the workflow, it PATCHes the workflow's `functionArgs` with the concrete values baked in, then triggers the workflow with an empty input.
...
The error message is exactly what JSON.parse returns when the second character of `[{{` is the second `{`: it has opened an array, opened an object, and is expecting a property name or the closing `}`. So the templates are not being evaluated in this code path.
...
**What would unblock builders.** Either fix the template engine so `{{...}}` is expanded inside `functionArgs` before JSON.parse (the actually-intended behavior), OR document the pre-execution PATCH workaround prominently. A template engine that renders parameters from the trigger payload is the only way a third party can reasonably call your workflows from outside, so this is the piece any agent integration depends on.

---

## Issue 5: KeeperHub's relayer wallet has zero gas by default, with no UX nudge to fund it

**Plain English.** The first time we got the workflow to encode calldata correctly, the action errored with `insufficient funds for intrinsic transaction cost`. We checked the balance of the relayer wallet KeeperHub uses for our org and it was zero. Nothing in the dashboard or the API responses told us we had to fund this wallet, and the relayer address is not displayed anywhere prominent in the UI. We found it because the error message included the `from` address.
...
1. On workflow creation, if the workflow contains any `web3/write-contract`, `web3/transfer-funds`, `web3/transfer-token`, or `web3/approve-token` action, surface an "execution wallet not funded" warning that links to the wallet's address and shows current balance per chain. A static "this workflow's relayer is `0x...`; fund it for X chain" notice would be enough.
2. The `GET /api/integrations` response should include `address` (full) and ideally `balance` per chain.
3. When an execution fails with `insufficient funds` for the relayer, the error should say "fund the KeeperHub relayer wallet at `0x...` on chain X" rather than bubbling up the JSON-RPC text.

---

## Issue 6: API endpoint paths do not follow REST conventions and are not documented

**Plain English.** Several KeeperHub API endpoints use paths that differ from what a developer would guess from the resource name. We discovered most of them by sending OPTIONS requests and reading the `Allow` headers, or by sending HTTP verbs and seeing which ones came back with `400` (real route, bad payload) instead of `404` or `405`.
...
**What would unblock builders.** Publish an OpenAPI spec at `/api/openapi.json` or `/api/docs`. Add a section to the GitHub README that lists the actual endpoint paths and verbs. Make the REST API mirror the MCP tools where it makes sense — an `execute_workflow` tool with no REST counterpart is confusing.

---

## Issue 5: API auth header convention is undocumented (Bearer vs raw key)

**Plain English.** The KeeperHub API key looks like a token but the docs do not say whether to put it in the `Authorization: Bearer <key>` header, an `X-API-Key` header, a query parameter, or a cookie. We tried each one. `Authorization: Bearer` worked.
...
- The visual workflow builder is good. We sketched the rescue flow there in three minutes before attempting it programmatically. The same workflow being exportable and triggerable via API is the right architecture.
- The MCP server is a real differentiator. Typing a workflow into Claude Code and having it created on KeeperHub is the experience that made us pick KeeperHub over a hand-rolled relayer.
- The pricing tier (free for hackathons, per-execution after) works for indie builders. Klaxon's whole pitch is that bonded individual operators can compete with VC-funded teams, and a KeeperHub-style execution layer with no minimum commitment fits that.
...
`lib.esm/index.mjs` re-exports symbols under aliased names that the
bundled chunk file doesn't actually export. `import { ... } from
...
sub-process it from Python. Anyone building a fully-ESM Node app (the
default for new projects) will hit this immediately.
...
what `index.mjs` re-exports, or document in the README that the SDK is
CJS-only today and add `"exports": { ".": { "require": "..." } }` to
`package.json` so bundlers don't try to resolve the broken ESM path.
...
intuitive order (provider, content, chatID) is wrong. The actual order
is provider, chatID, content. We spent half an hour seeing
`verified=false` from a provider that was actually fine because we'd put
the wrong thing in the middle slot.
...
TypeScript types are `(string, string, string)` so the compiler doesn't
catch it.
...
README showing `chatCompletion → processResponse` with the chat ID
...
Compute, the identifier you pass into `processResponse` is in a custom
HTTP response header (`ZG-Res-Key`), not in the body. We used the body
`id` first, got `verified=false` consistently, and lost about 20 minutes
before we noticed the header.
...
themselves (e.g. for streaming or header inspection), document the
header name and casing in the README.
...
third request comes back as HTTP 429 ("too many concurrent") or, worse,
as a TCP socket hang-up mid-request. Without retry, that agent's finding
has no TEE envelope and the other two agents reject it on receive.
Quorum drops from 3 to 2 and the rescue can't fire.
...
within 180s. That is an external-dependency reality, not a Klaxon bug.

**What would unblock builders.**

1. Document the per-user concurrency cap in the dstack provider's
   metadata so callers can rate-limit themselves up front.
2. Return HTTP 429 (with `Retry-After`) consistently rather than killing
   the TCP socket. Socket hang-ups make every HTTP client think they hit
   a network glitch and either fail-fast or retry too aggressively.
3. Add a per-user request queue server-side, or expose one in the SDK,
   so callers don't have to roll their own backoff.
4. Provide more than one attested-chatbot provider in the catalog.
   Today `list-providers` returns only one chatbot provider and one
   image-editing provider; if the chatbot provider is degraded there is
   no fallback.
...
`open-jsonrpc-provider`, which depends on a version of `axios` that is
...
did not file separate GitHub issues because the fix is on
...
## What worked well with 0G Compute

- The local-verification model (no provider round-trip on the receive
  path) is the right design for a swarm. Once the TEE envelope was
  attached to the Finding, every agent in the swarm could verify it
  with `processResponse` against `(tee_text, tee_signature, signing_address)`
  with no extra trust in the provider and no added latency on the
  gossip path. That made gating quorum on the TEE attestation feasible.
- The dstack TEE flow is interesting. `keccak256(tee_text)` as the
  on-chain commitment of *which model reviewed the finding* is something
  you can build slashing on top of.
- The broker's auto-deposit + auto-acknowledge flow is easy to use. We
  topped up the deposit account once (3 OG) and didn't think about it
